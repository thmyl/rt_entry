...
def parse_log(file_path):
    ...
    # 2. 提取 recall100@1024
    computation = re.compile(r"average dimensions per distance computation\s*=\s*([\d\.]+)")
    ...
    try:
        with open(file_path, 'r') as f:
            for line in f:
                ...
                # --- 步骤2: 提取 Recall ---
                if "average dimensions per distance computation" in line:
                    match_r = computation.search(line)
                    if match_r:

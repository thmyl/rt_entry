...
# 数据集名称
datasets = ['DEEP1M', 'DEEP10M', 'SIFT1M', 'SIFT10M', 'COCO-I2I', 'GIST']
methods = ['PARS-Base', 'PARS', 'GAGRA', 'CAGRA+RT', 'GGNN', 'GANNS']
...
for i, (x_data, y_data) in enumerate(all_groups[18:24]):
    ...
    axs[row, col].set_title(datasets[subplot_id], fontsize=sz)
    # axs[row, col].set_ylabel('Queries per Second', fontsize=sz)  # 右列不显示
    axs[row, col].grid(True, linestyle='--')
    # axs[row, col].legend()
...
# 调整布局
plt.tight_layout()
fig.legend(methods, loc='upper center', bbox_to_anchor=(0.5, 1.07), ncol=3, frameon=False, fontsize=sz-2)
plt.show()
plt.savefig("./img/3x2.png", format="png", bbox_inches="tight")
print("图像已保存到文件：./img/3x2.png")
plt.savefig("./img/3x2.pdf", format="pdf", bbox_inches="tight")
print("图像已保存到文件：./img/3x2.pdf")
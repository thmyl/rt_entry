...
PCA::PCA(float *datasets, uint _nb, uint _dim) {
	nb = _nb;
	dim = _dim;
	printf("nb = %d, d = %d\n", nb, dim);
	B.resize(nb, dim);
...
void PCA::computeEig(Eigen::MatrixXd &C, Eigen::MatrixXd &vec, Eigen::RowVectorXd &val)
{
	// printf("computeEig\n");
...
void PCA::reset(const Eigen::MatrixXd& data, uint _dim, std::vector<uint>& idlist) {//重置数据
	nb = idlist.size();
	dim = _dim;
	B.resize(nb, dim);
	C.resize(dim, dim);
	for (int i = 0; i < nb; i++) {
		for (int j = 0; j < dim; j++) {
			// B(i, j) = data[1LL * idlist[i] * dim + j];
			B(i, j) = data(idlist[i], j);
		}
	}
	calc_eigenvalues();
}

void PCA::pca_transform(const Eigen::MatrixXd &_data, Eigen::MatrixXd &res, uint pj_dim){
	Eigen::MatrixXd data = _data.rowwise() - meanvecRow;
	res = data * vec.leftCols(pj_dim);
}

void PCA::pca_inverse_transform(const Eigen::MatrixXd &data, Eigen::MatrixXd &res, uint pj_dim){
	res = data * vec.leftCols(pj_dim).transpose();
	res.rowwise() += meanvecRow;
}
...
void PCA::calc_result(uint pj_dim){
	// for(int i=0; i<dim; i++) printf("%.2f ", val(i));
	// printf("\n");

...
datasets = {
    "SIFT1M": {
        "y_values": [88, 62, 47, 38, 31, 26],
        "top_line": 128,
        # "title": "SIFT-1M"
    },
    "DEEP1M": {
        "y_values": [84, 69, 57, 48, 41, 35],
        "top_line": 96,
        # "title": "DEEP-1M"
    },
    "GIST": {
        "y_values": [434, 238, 149, 100, 73, 53],
        "top_line": 960,
        # "title": "GIST"
    }
}
...
for idx, (ax, (title, data)) in enumerate(zip(axes, datasets.items())):
    ...
    # 放大坐标字体
    ax.tick_params(axis='y', labelsize=sz-12)
    ax.tick_params(axis='x', labelsize=sz-12)

    # 添加标签
    ax.set_xlabel("$m$", fontsize=sz, math_fontfamily='cm')
...
fig.legend(handles=handles, labels=["Original dimension"], loc='upper center', ncol=1,
           bbox_to_anchor=(0.5, 1.04), fontsize=sz, frameon=False)

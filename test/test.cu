...
void test_gemm(cublasHandle_t handle, int m, int n, int k, T *A, T *B, S *C,
    ...
    for (int i = 0; i < iteration; ++i) {
        ...
        if(success <= 0) {
            printf("Error in cublas_gemm_ex\n");
            return;
        }
    }
    if (total_time > 0)
...
int main() {
    ...
    float* d_fB;
    float* d_fC;
    cudaMalloc(&d_fA, m * k * sizeof(float));
    cudaMalloc(&d_fB, k * n * sizeof(float));
    cudaMalloc(&d_fC, m * n * sizeof(float));
    cudaMemcpy(d_fA, fA, m * k * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_fB, fB, k * n * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_fC, fC, m * n * sizeof(float), cudaMemcpyHostToDevice);
    
    printf(">>>>>>>>>>>>>>>>> test fp32 >>>>>>>>>>>>>>>>>\n");
    // for (int algo = start_algo; algo <= end_algo; ++algo)
    //     test_gemm(handle, m, n, k, d_fA, d_fB, d_fC, &f_alpha, &f_beta, algo, iteration);
